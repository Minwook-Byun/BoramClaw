...
TOOL_SPEC = {
    "name": "google_gmail_pull",
    "description": "P1 scaffold for Gmail label pull and sync-run logging.",
    "version": __version__,
    "input_schema": {
        "type": "object",
        "properties": {
            ...
            "label": {"type": "string", "description": "e.g., VC-REPORT"},
...
def run(input_data: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    max_messages = int(input_data.get("max_messages", 500) or 500)
    max_messages = max(1, min(max_messages, 2000))
    label = str(input_data.get("label", "VC-REPORT")).strip() or "VC-REPORT"
    ...
    store.create_integration_sync_run(
        ...
        window_to=window_to,
        status="running",
        summary={"planned_max_messages": max_messages, "label": label},
    )
    ...
    summary = {
        ...
        "planned_max_messages": max_messages,
        "dry_run": dry_run,
        "label": label,
        "auto_refresh": auto_refresh,
        "refreshed": bool((refresh_result or {}).get("refreshed", False)),
        "notes": "P1 scaffold: Gmail API fetch is not wired yet.",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="google_gmail_pull cli")
